...
from sklearn.metrics import accuracy_score #导入
from sklearn.metrics import precision_score,recall_score #导入精确度，召回率模块
...
input_file="C://Users//lenovo//Desktop//dataming//训练集.xlsx"
...
yp =tree_model.predict(x_train) #分类预测
cm_plot(y_train,yp).show() #显示混淆矩阵可视化结果


yp=tree_model.predict(x_test) #分类预测
cm_plot(y_test,yp).show() #显示混淆矩阵可视化结果

from sklearn.metrics import roc_curve #导入ROC曲线函数

fpr, tpr, thresholds = roc_curve(y_train,tree_model.predict_proba(x_train)[:,1], pos_label=1)
plt.plot(fpr, tpr, linewidth=2, label = 'ROC of tree_train', color = 'green') #作出ROC曲线
...
fpr, tpr, thresholds = roc_curve(y_test,tree_model.predict_proba(x_test)[:,1], pos_label=1)
plt.plot(fpr, tpr, linewidth=2, label = 'ROC of tree_test', color = 'green') #作出ROC曲线
plt.xlabel('False Positive Rate') #坐标轴标签
plt.ylabel('True Positive Rate') #坐标轴标签
plt.ylim(0,1.05) #边界范围
plt.xlim(0,1.05) #边界范围
...
train_rf_predictions = tree_model.predict(x_train)
train_rf_probs = tree_model.predict_proba(x_train)[:, 1]

rf_predictions = tree_model.predict(x_test)
rf_probs = tree_model.predict_proba(x_test)[:, 1]
